...
2.  Then merge/unify the underlying Rust bindings with the R-side
    engine.
...
- `src/rust/`
  ...
  - It contains the recent MLT fixes, conformance tests against
    `mlt-core`, `base_zoom`, buffered tile assignment, spatial ordering,
    and newer encoder corrections.
- `python/`
  - A separate `pyo3` crate plus Python package wrapper.
  - This is not yet aligned with the R-side Rust core.
  - It appears to be a fork/copy of the Rust engine, not a shared
    library.

That duplication is the core technical debt. Do not try to solve it
first. First get Python correct and stable.

## What Exists On The Python Side

Python package layout:

- [`python/pyproject.toml`](https://walker-data.com/Users/kylewalker/Library/CloudStorage/Dropbox/dev/freestiler/python/pyproject.toml)
- [`python/Cargo.toml`](https://walker-data.com/Users/kylewalker/Library/CloudStorage/Dropbox/dev/freestiler/python/Cargo.toml)
- [`python/src/lib.rs`](https://walker-data.com/Users/kylewalker/Library/CloudStorage/Dropbox/dev/freestiler/python/src/lib.rs)
- [`python/python/freestiler/__init__.py`](https://walker-data.com/Users/kylewalker/Library/CloudStorage/Dropbox/dev/freestiler/python/python/freestiler/__init__.py)
- [`python/tests/`](https://walker-data.com/Users/kylewalker/Library/CloudStorage/Dropbox/dev/freestiler/python/tests)
...
- Python preprocessing happens in
  [`python/python/freestiler/__init__.py`](https://walker-data.com/Users/kylewalker/Library/CloudStorage/Dropbox/dev/freestiler/python/python/freestiler/__init__.py).
...
- Attributes are passed as typed column lists:
  ...
  - `float_columns`
  - `bool_columns`
- The Rust entry point is `_freestile()` in
  [`python/src/lib.rs`](https://walker-data.com/Users/kylewalker/Library/CloudStorage/Dropbox/dev/freestiler/python/src/lib.rs).

This is a decent architecture for Python. The immediate problem is not
the API shape. The problem is drift from `src/rust/`.

## Important Drift From The Main Rust Engine

Assume `python/src/*.rs` is behind `src/rust/src/*.rs` until proven
otherwise.

The R-side Rust engine now includes work that Python likely does not
have yet:
...
- [`python/src/mlt.rs`](https://walker-data.com/Users/kylewalker/Library/CloudStorage/Dropbox/dev/freestiler/python/src/mlt.rs)
- [`python/src/tiler.rs`](https://walker-data.com/Users/kylewalker/Library/CloudStorage/Dropbox/dev/freestiler/python/src/tiler.rs)
- [`python/src/lib.rs`](https://walker-data.com/Users/kylewalker/Library/CloudStorage/Dropbox/dev/freestiler/python/src/lib.rs)

Before doing anything ambitious, diff those against:

- [`src/rust/src/mlt.rs`](https://walker-data.com/Users/kylewalker/Library/CloudStorage/Dropbox/dev/freestiler/src/rust/src/mlt.rs)
- [`src/rust/src/tiler.rs`](https://walker-data.com/Users/kylewalker/Library/CloudStorage/Dropbox/dev/freestiler/src/rust/src/tiler.rs)
- [`src/rust/src/lib.rs`](https://walker-data.com/Users/kylewalker/Library/CloudStorage/Dropbox/dev/freestiler/src/rust/src/lib.rs)
...
First make the Python package correct, installable, and passing
meaningful tests.
...
3.  Compare Python behavior against the R-side engine on a few small
    fixtures.
4.  Bring over only the minimum Rust fixes needed for Python
    correctness.
...
Once Python is buildable and testable, audit feature parity against the
R-side engine.
...
1.  `base_zoom`
    ...
    - R-side does.
    - If Python is meant to match R, add it after correctness is
      established.
...
When you get to the merge, do not try to unify both codepaths by manual
copy/paste.

Instead:

1.  Treat `src/rust/` as the source of truth for core logic.
2.  Identify the Python-specific pieces in
    [`python/src/lib.rs`](https://walker-data.com/Users/kylewalker/Library/CloudStorage/Dropbox/dev/freestiler/python/src/lib.rs):
    - WKB parsing
    - Python dict extraction
    - PyO3 function signatures
3.  Extract a pure Rust engine API that accepts already-parsed
    `LayerData`.
...
That is reasonable, but compare it with the R-side logic. The R package
already had CRS edge-case fixes, so Python should not regress there.

### 2. Geometry transport

Python uses WKB to cross the binding boundary. That is fine.

Do not replace it casually. WKB is a sensible Python-side transport
because:
...
If anything, this is a good argument for a shared core that accepts
parsed `Feature`s, not for forcing Python to mimic the R input protocol.
...
- [`python/tests/test_basic.py`](https://walker-data.com/Users/kylewalker/Library/CloudStorage/Dropbox/dev/freestiler/python/tests/test_basic.py)
- [`python/tests/test_features.py`](https://walker-data.com/Users/kylewalker/Library/CloudStorage/Dropbox/dev/freestiler/python/tests/test_features.py)
- [`python/tests/test_multilayer.py`](https://walker-data.com/Users/kylewalker/Library/CloudStorage/Dropbox/dev/freestiler/python/tests/test_multilayer.py)
...
There are generated binaries and sample PMTiles under `python/` right
now:
...
For MLT behavior, the current best source of truth in this repo is the
R-side Rust implementation:
...
If Python disagrees with that code, assume Python is wrong until proven
otherwise.
